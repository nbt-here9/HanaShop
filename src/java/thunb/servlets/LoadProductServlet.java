...
public class LoadProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ConstantsKey.UPDATE_PRODUCT_PAGE;
        try {
            ...
            if (txtProductID != null && !txtProductID.trim().isEmpty()
                ...
                boolean rs = productsDAO.loadSelectedProduct(productID);
                if (rs) {
                    request.setAttribute("LOADED_PRODUCT", productsDAO.getSelectedProduct());
                }
            }
        } catch (SQLException ex) {
            log("LoadProductServlet_SQLException:" + ex.getMessage());
        } catch (NamingException ex) {
            log("LoadProductServlet_NamingException:" + ex.getMessage());
        } finally {
